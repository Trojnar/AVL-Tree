...
class TreeNode:
    ...
    def __insert(self, node, min_depth, depth=1, inserted=False):
        print(self, inserted, min_depth, depth)
        ...
        if not inserted:
            inserted = TreeNode.__insert(self.left, node, min_depth, depth + 1, inserted)  # type: ignore
            if not inserted:
                inserted = TreeNode.__insert(self.right, node, min_depth, depth + 1, inserted)  # type: ignore
        print(3)
        if inserted:
            return True
        else:
...
class TreeMap:
    ...
    def parse_tuple(cls, tree_t: tuple) -> TreeMap:
        """
        Parse given tuple organized in form (left_subtree, key, right_subtree).
